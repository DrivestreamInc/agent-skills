<#
  ...
  install-agent-mcps.ps1 so you do not need a local clone.
#>
[CmdletBinding()]
param(
...
$tmp = Join-Path ([System.IO.Path]::GetTempPath()) ("agent-mcps-dl-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $extractRoot = Get-ChildItem -LiteralPath $tmp -Directory | Where-Object {
        Test-Path -LiteralPath (Join-Path $_.FullName "tools\install-agent-mcps.ps1")
    } | Select-Object -First 1

    if (-not $extractRoot) {
        Write-Error "Archive did not contain expected layout (missing tools/install-agent-mcps.ps1)."
        exit 1
    }

    $installScript = Join-Path $extractRoot.FullName "tools\install-agent-mcps.ps1"
    $installArgs = @{
